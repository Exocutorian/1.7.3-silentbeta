...
The repo tracks the (modified) sources but not the downloaded jars,
libraries and natives — those come from RetroMCP's `setup`. **Careful:**
`setup` (and `decompile`) recreate the `minecraft*/` folders, wiping the
Silent Beta sources — restore them from git right after:
...
2. From the repository root run:
   ...
   git checkout .                                 # restore Silent Beta sources it wiped
   java -jar RetroMCP-Java-CLI.jar recompile
   ```

Do **not** run `decompile` in this repo — the sources are already here
and it would overwrite them with vanilla ones (if it happens: `git checkout .`).
...
regenerated by RetroMCP on first run. If `recompile` fails with
"package org.lwjgl does not exist", `libraries/` is missing — run the
setup step above.
